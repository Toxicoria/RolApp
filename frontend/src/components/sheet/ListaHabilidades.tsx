...
export default function ListaHabilidades({ habilidades, alCambiar, alEliminar }: Props) {
  const [expandidaId, setExpandidaId] = useState<string | null>(null);
  return (
    <div className="overflow-y-auto no-scrollbar h-full p-3">
      <div className="space-y-2">
        {habilidades.map((hab) => (
          <div key={hab.id} className="border border-gray-200 rounded-lg overflow-hidden shadow-sm bg-white">
            <div className="flex items-center gap-2 p-2 group">
              <input
                type="number" value={hab.valor}
              ...
              />
              <button
                onClick={() => setExpandidaId(expandidaId === hab.id ? null : hab.id)}
            ...
            {expandidaId === hab.id && (
              <div className="p-3 bg-gray-50 border-t border-gray-200 space-y-3">
                <input
                  type="text" placeholder="Nombre" value={hab.nombre}
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  >
                    {Object.values(FormaArea).map((v) => <option key={v} value={v}>{v}</option>)}
                  </select>
                  <input
                    type="text" placeholder="Alcance" value={hab.alcance}
                ...
                <textarea
                  ...
                  placeholder="Descripción..." className="w-full text-xs p-2 border rounded bg-white resize-none" rows={3}
                />
              </div>
            )}
          </div>
