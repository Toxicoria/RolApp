...
export default function Inventario({ items, alCambiarItem, alEliminarItem, alAgregarSubItem, alCambiarSubItem, alEliminarSubItem }: Props) {
  return (
        ...
        {items.map((item) => (
          <div key={item.id} className="border border-gray-200 rounded-lg p-3 bg-gray-50 shadow-inner">
            <div className="flex items-center gap-2 mb-2">
              ...
              />
              <input
                type="text" value={item.nombre} placeholder="Item..."
              ...
              <div className="flex items-center gap-1 bg-gray-200 px-2 py-1 rounded">
                <input type="checkbox" checked={item.esContenedor}
              ...
              <div className="ml-6 mt-2 border-l-2 border-blue-200 pl-3 space-y-2">
                {item.contenido?.map((sub) => (
                  <div key={sub.id} className="flex items-center gap-2 group">
                    ...
                    />
                    <input
                      type="text" value={sub.nombre} placeholder="Sub-item..."
